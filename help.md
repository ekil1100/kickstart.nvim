...
| 命令 | 功能 |
|------|------|
| `:lua vim.pack.update(nil, { offline = true })` | 查看插件状态 (`q` 退出) |
| `:lua vim.pack.update()` | 获取并选择插件更新 |
| `:Mason` | LSP/工具管理 (`q` 退出) |
...
`vim.pack` 直接通过 Git 下载插件：

- 查看插件状态：`:lua vim.pack.update(nil, { offline = true })`
- 获取更新：`:lua vim.pack.update()`，在预览页用 `:write` 应用、`:quit` 取消
- 删除不再配置的插件：先从配置移除，再执行 `:lua vim.pack.del({ '插件名' })`
